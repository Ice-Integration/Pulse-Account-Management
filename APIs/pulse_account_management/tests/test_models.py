...
from APIs.pulse_account_management.SimulationEngine.common_models import (
    ActiveFeature,
    Address,
    BaseAccountDetails,
...
from APIs.pulse_account_management.SimulationEngine.endpoint_models import (
...
class TestAccountManagementModels(unittest.TestCase):
    """
    ...
    def test_active_feature_model(self):
        ...
        feature = ActiveFeature(
            featureId="FEAT_INTL_CALL", featureName="International Calling Pass", monthlyCost=15.00
        )
    ...
    def test_customer_account_details_model(self):
        """Test BaseAccountDetails model with nested objects."""
        ...
        feature = ActiveFeature(featureId="F1", featureName="Feature 1", monthlyCost=10.00)
        service = ServicePlan(
            serviceId="SVC1",
            planName="Basic Plan",
        ...
        account = BaseAccountDetails(
            ...
            billingAddress=billing_address.model_dump(mode="json"),
            communicationPreferences=comm_prefs.model_dump(mode="json"),
            ...
            isVerified=True,
            securityPinSet=True,
            serviceAddress=service_address.model_dump(mode="json"),
            services=[service.model_dump(mode="json")],
        )
    ...
    def test_device_upgrade_eligibility_model(self):
        ...
        self.assertEqual(
            eligibility.earlyUpgradeOptions, "Eligible for early upgrade with a $150 fee."
        )
    ...
    def test_service_modification_response_model(self):
        ...
        self.assertEqual(
            response.message, "Your plan has been successfully changed to Unlimited Pro."
        )
        self.assertEqual(
            response.nextBillImpactEstimate, "A prorated charge of $5.50 will be added."
        )
    ...
    def test_knowledge_base_query_response_model(self):
        ...
        self.assertEqual(
            response.answer, "The direct answer to the query, synthesized from the knowledge base."
        )
