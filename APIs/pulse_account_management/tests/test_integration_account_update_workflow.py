...
# Assuming unittest.TestCase is in a common_utils package
# As per instructions, the test suite MUST inherit from this base class.


class TestAccountUpdateWorkflowIntegration(unittest.TestCase):
    """
    ...
    def setUp(self):
        ...
        db.DB["availablePlans"]["plans"] = {
            self.initial_plan_id: {
                "id": self.initial_plan_id,
                "name": "Basic Connect",
                "description": "Entry-level plan used as the starting state for the workflow test.",
                "monthlyCost": 35.00,
                "type": "PLAN",
                "dataAllowance": "10GB",
            },
    ...
    def test_complete_account_update_workflow(self):
        ...
        update_request = {
            ...
            "communicationPreferences": {"emailOptIn": True, "smsOptIn": True},
        }

        update_result = update_account_information(
            accountId=self.account_id, requestBody=update_request
        )
        ...
        eligibility_result = check_device_upgrade_eligibility(
            accountId=self.account_id, identifier=self.line_number, identifierType="LINE_NUMBER"
        )
        ...
        self.assertIsNone(
            eligibility_result.get("reason"), "No eligibility blocking reason expected"
        )
        ...
        self.assertIn(
            "SVC_WORKFLOW_01", [svc["serviceId"] for svc in services], "Service should exist"
        )
        ...
        self.assertTrue(
            any(f.get("featureId") == self.feature_id for f in active_features),
            "International calling feature should be active",
        )
    ...
    def test_workflow_error_handling(self):
        ...
        with self.assertRaisesRegex(
            ValueError,
            re.escape("Invalid email value 'invalid-email-format' for field 'contactEmail'"),
        ):
            update_account_information(
                accountId=self.account_id, requestBody={"contactEmail": "invalid-email-format"}
            )

        # Test 2: Invalid phone number update

        with self.assertRaisesRegex(
            ValidationError, re.escape("The phone number 'invalid-phone-123' is not valid.")
        ):
            update_account_information(
                accountId=self.account_id, requestBody={"contactPhone": "invalid-phone-123"}
            )

        # Test 3: Checking eligibility for non-existent device
        with self.assertRaisesRegex(
            ValidationError,
            re.escape("Device with LINE_NUMBER '555-999-9000' not found on account."),
        ):
            check_device_upgrade_eligibility(
                accountId=self.account_id, identifier="555-999-9000", identifierType="LINE_NUMBER"
            )

        # Test 4: Changing to non-existent plan
        with self.assertRaisesRegex(
            ValidationError, re.escape("Item NONEXISTENT_PLAN_XYZ not found in the database.")
        ):
            modify_service_plan_or_feature(
                accountId=self.account_id,
                action="CHANGE_PLAN",
        ...
        with self.assertRaisesRegex(
            ValidationError, re.escape("Item NONEXISTENT_FEATURE_ABC not found in the database.")
        ):
            modify_service_plan_or_feature(
                accountId=self.account_id,
                action="ADD_FEATURE",
    ...
    def test_workflow_step_by_step_validation(self):
        ...
        update_result = update_account_information(
            accountId=self.account_id, requestBody={"contactEmail": new_email}
        )
        ...
        self.assertTrue(
            any(f.get("featureId") == self.feature_id for f in final_service["activeFeatures"])
        )
