...
from pulse_account_management.account_management import query_available_plans_and_features
...
class TestQueryAvailablePlansAndFeatures(AccountManagementBaseTestCase):
    ...
    def setUp(self):
        ...
        self.mock_patcher = patch(
            "pulse_account_management.SimulationEngine.utils._get_gemini_response"
        )
    ...
    def test_query_international_travel_addons(self):
        ...
        snippet_content = " ".join([
            s.get("title", "") + " " + s.get("text", "") for s in result["snippets"]
        ])
        self.assertTrue(
            "International" in snippet_content, "Expected international features in results"
        )
    ...
    def test_query_available_service_plans_and_differences(self):
        ...
        result = query_available_plans_and_features(
            "available Sunday Mobile service plans and differences"
        )
    ...
    def test_query_non_string_input(self):
        ...
        with self.assertRaisesRegex(CustomValidationError, "query must be a string."):
            query_available_plans_and_features(query=123)

    def test_query_none_input(self):
        """Test that None query raises ValueError."""
        with self.assertRaisesRegex(CustomValidationError, "query must be a string."):
            query_available_plans_and_features(query=None)
    ...
    def test_query_nonexistent_plan(self):
        ...
        self.assertTrue(
            ...
            or "no information" in result.get("answer", "").lower()
        )
