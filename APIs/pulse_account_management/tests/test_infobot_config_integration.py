"""
Test suite for Pulse Account Management Infobot Configuration Integration

Tests the integration of the new configuration system with pulse_account_management service.
"""
...
from pulse_account_management.SimulationEngine.utils import (
    get_infobot_config,
...
class TestInfobotConfigIntegration(unittest.TestCase):
    ...
    def test_get_token_missing_service_account_info(self):
        ...
        with self.assertRaisesRegex(
            ValueError, re.escape("Service account info must be set before uploading the image.")
        ):
            utils._get_token()

    @patch("pulse_account_management.SimulationEngine.utils.service_account")
    @patch("pulse_account_management.SimulationEngine.utils.requests.Session")
    def test_get_token_success_with_config(self, mock_session_class, mock_service_account):
        ...
        encoded_info = base64.b64encode(json.dumps(service_account_info).encode("utf-8")).decode(
            "utf-8"
        )
    ...
    def test_get_token_invalid_json(self):
        ...
        with self.assertRaisesRegex(
            json.JSONDecodeError, re.escape("Expecting value: line 1 column 1 (char 0)")
        ):
            utils._get_token()

    # Tests for _query_infobot with new config system
    @patch("pulse_account_management.SimulationEngine.utils._get_token")
    @patch("pulse_account_management.SimulationEngine.utils.requests.post")
    def test_query_infobot_success(self, mock_post, mock_get_token):
        ...
        mock_response.json.return_value = {"response": {"answer": "Test response", "snippets": []}}
    ...
    def test_query_infobot_missing_tool_resource(self):
        ...
        with self.assertRaisesRegex(
            ValueError, re.escape(f"Tool resource '{tool_name}' not configured")
        ):
            utils._query_infobot(query=query, tool_name=tool_name)
    ...
    def test_query_infobot_http_error(self, mock_post, mock_get_token):
        ...
        # Test the function
        with self.assertRaisesRegex(requests.exceptions.HTTPError, re.escape("404 Not Found")):
            utils._query_infobot("test query", "account_orders")
    ...
    def test_query_infobot_connection_error(self, mock_post, mock_get_token):
        ...
        # Test the function
        with self.assertRaisesRegex(
            requests.exceptions.ConnectionError, re.escape("Connection failed")
        ):
            utils._query_infobot("test query", "account_orders")

    # Tests for query_plans_and_features_infobot
    @patch("pulse_account_management.SimulationEngine.utils._query_infobot")
    def test_query_plans_and_features_infobot(self, mock_query_infobot):
        ...
        mock_query_infobot.assert_called_once_with("What plans are available?", "plans_features")
    ...
    def test_query_account_orders_infobot(self, mock_query_infobot):
        ...
        self.assertEqual(result, expected_response)
        mock_query_infobot.assert_called_once_with("What is my order status?", "account_orders")
    ...
    def test_config_update_affects_queries(self):
        ...
        update_infobot_config(gcp_project="custom-project", location="custom-location")
...
class TestInfobotConfigDefaults(unittest.TestCase):
    """Test that default configuration values are correct for pulse_account_management"""
    ...
    def test_default_tool_resources(self):
        ...
        self.assertIn("plans_features", config.tool_resources)
        self.assertEqual(
            config.tool_resources["account_orders"], "46f527f8-0509-4e28-9563-db5666e0790b"
        )
        self.assertEqual(
            config.tool_resources["plans_features"], "c90c11bb-6868-4631-8bf0-7f8b5fe4b92c"
        )
