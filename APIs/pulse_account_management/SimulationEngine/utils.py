...
import requests
from common_utils.pulse_infobot_config import CESInfobotConfigManager
...
from pulse_account_management.SimulationEngine.phone_utils import normalize_phone_number

load_dotenv()


def validate_non_empty_string(v: Any, info: ValidationInfo) -> str:
    """Reusable Pydantic mode='before' field_validator: checks type, length, and whitespace."""
    if not isinstance(v, str):
        raise ValidationError(f"{info.field_name} must be a string.")
    if len(v) < 1:
        raise ValidationError(f"{info.field_name} must be at least 1 characters long.")
    if not v.strip():
        raise ValidationError(f"{info.field_name} must be a non-empty string.")
    return v
...
def _get_token() -> str:
    ...
    if not config.service_account_info:
        raise ValueError("Service account info must be set before uploading the image.")
    info = json.loads(base64.b64decode(config.service_account_info).decode("utf-8"))
    credentials = service_account.Credentials.from_service_account_info(info, scopes=config.scopes)
...
def add_device_to_account(account_id: str, device_data: Dict[str, Any]) -> Dict[str, Any]:
...
def add_service_to_account(account_id: str, service_data: Dict[str, Any]) -> Dict[str, Any]:
...
def update_service_plan(account_id: str, service_index: int, service_data: Dict[str, Any]) -> None:
...
def _get_gemini_response(query: str) -> str:
    ...
    model_name = os.getenv("DEFAULT_GEMINI_MODEL_NAME") or os.getenv("GEMINI_MODEL_NAME")
    if not model_name:
        raise EnvironmentError(
            "Gemini API Model Name not found. Please create a .env file in the project root with DEFAULT_GEMINI_MODEL_NAME or GEMINI_MODEL_NAME, or set it as an environment variable."
        )
...
def get_model_from_gemini_response(
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            print_log(f"Error validating model (attempt {attempt + 1}/{max_retries + 1}): {e}")
...
def search_plans_by_query(query: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        from pulse_account_management.SimulationEngine.endpoint_models import PlanSearchResult
        ...
        for plan_data in json_response:
            try:
                ...
                print_log(f"Error validating plan data: {e}, skipping plan: {plan_data}")
                continue

        print_log(f"Found {len(result_plans)} matching plans/features for query: {query}")
        return result_plans
...
def search_account_orders_by_query(query: str, account_id: str) -> List[Dict[str, Any]]:
    ...
    """
    from pulse_account_management.SimulationEngine.search_engine import search_engine_manager
...
def save_infobot_config(filepath: str = None):
    ...
    Args:
        filepath: Optional path to save config. Defaults to pulse_account_management_infobot_config.json
    """
