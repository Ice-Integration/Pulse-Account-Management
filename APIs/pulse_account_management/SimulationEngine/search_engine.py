...
class ServiceAdapter(Adapter):
    """Adapter for converting Pulse Account Management database into searchable documents."""

    def db_to_searchable_documents(self) -> List[SearchableDocument]:
        """Convert the Pulse Account Management database into searchable documents."""
    ...
    def _create_order_document(
        ...
        if order_data.get("statusDescription"):
        ...
        if order_data.get("estimatedCompletionDate"):
            text_content_parts.append(
                f"Estimated completion: {order_data['estimatedCompletionDate']}"
            )
        ...
        metadata = {
            ...
            "content_type": "order",
            "service": "pulse_account_management",
        }
...
search_engine_manager = search_engine_manager.get_engine_manager("pulse_account_management")
