...
from app.safety import is_unsafe_prompt
...
async def retrieve(question: str, limit: int = 5) -> list[dict[str, Any]]:
    ...
    try:
        ...
        rows = await conn.fetch(
            "SELECT id,title,content,metadata,1-(embedding <=> $1::vector) score "
        ...
        if not rows:
            rows = await conn.fetch(
                "SELECT id,title,content,metadata,0.0 score FROM knowledge_documents "
...
async def ask(request: AskRequest) -> dict[str, Any]:
    ...
    response = await client.responses.create(
        ...
        input=[
            {
                "role": "system",
                "content": "You are Pulse Support. Answer only from supplied sources. Never invent account facts. If sources are insufficient, say so. Cite sources as [1], [2].",
            },
    ...
    return {
        ...
        "citations": [
            {"id": str(d['id']), "title": d['title'], "score": float(d['score'])}
            for d in docs
        ],
    }
